...
int main(int argc, char* argv[]) {
  if (argc < 3) {
    cout << "Remote Control Dandy PAC archive unpacker" << endl;
    cout << "Usage: " << argv[0] << " [pac] [outprefix]" << endl;
    
    return 0;
  }
  
  string pacName(argv[1]);
  string outprefix(argv[2]);
  
  TBufStream ifs;
  ifs.open(pacName.c_str());
  
  DandySmallPac::unpack(ifs, outprefix);
